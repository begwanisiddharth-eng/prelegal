...
/** Parse a raw text string with **bold** markers and <span> placeholders into segments. */
export type TextSegment = { text: string; bold: boolean }

export function parseSegments(raw: string, replacements: Record<string, string>): TextSegment[] {
  let text = raw
  for (const [key, value] of Object.entries(replacements)) {
    text = text.replaceAll(`<span class="coverpage_link">${key}</span>`, value)
  }
